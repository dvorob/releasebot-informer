...
def how_many_is_working() -> str:
    """
        ...
        :return: msg
    """
    issues_releases = JiraIssues().jira_search(config.issues_not_closed_resolved)
    issues_working_now = JiraIssues().jira_search(config.search_issues_work)
        ...
        for issue in now_db:
            if issue not in now_id:
                ...
                if str(j_issue.fields.resolution) == 'Выполнен':
                    ...
                    config_deploy = Spike.read(item='deploy', aerospike_set='remaster')
...
def collect_success_releases(issue_name):
    ...
    try:
        ...
        if release_name_version['version'] is False:
            ...
            components_releases_success = Spike.read(item='auto_rollback',
            ...
            logger.info('collect_success_releases, response: %s',
                        Spike.write(item='auto_rollback',
